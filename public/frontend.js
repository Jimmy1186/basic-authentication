...
ourForm.addEventListener("submit", async e => {
  e.preventDefault()
  const response = await axios.post("/jwt-login", { username: usernameField.value, password: passwordField.value })
...
document.getElementById("getsecret").addEventListener("click", async function () {
  const response = await axios.post("/jwt-secret", { token: localStorage.getItem("ourToken") })
  if (response.data.status == "success") {
